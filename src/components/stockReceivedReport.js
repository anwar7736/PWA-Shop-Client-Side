...
class StockReceivedList extends React.Component{
	constructor() {
        ...
        componentDidMount=()=>{
         Axios.get('https://shop-api.coderanwar.online/api/ReceivedAllStockData')
         .then(response=>{
      ...
      filterByDate=()=>{
           ...
           if(from_date=='' || to_date=='')
           {
             cogoToast.warn('Both dates are required!');
              ...
              Axios.post('https://shop-api.coderanwar.online/api/ReportFilterByDate', {from_date:from_date, to_date:to_date})
             .then(response=>{
                 this.setState({dataTable : response.data[0], total: response.data[1]});
             })
             .catch(error=>{
